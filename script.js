...
// ===============================
// 🌸 FLOATING SAI NAME (ALL PAGES EXCEPT SAD)
// ===============================
function createFloatingSai(){
    let sai=document.createElement("div");
    sai.innerHTML="Sai 💖";
    sai.style.position="fixed";
    sai.style.top=Math.random()*80+"%";
    sai.style.left=Math.random()*90+"%";
    sai.style.fontSize="36px";
    sai.style.fontFamily="'Caveat', cursive";
    sai.style.fontWeight="bold";
    sai.style.color="#000"; // dark black
    sai.style.textShadow="0 0 15px #000, 0 0 35px #111";
    sai.style.zIndex=9999;
    sai.style.cursor="default";
    document.body.appendChild(sai);

    let directionX=Math.random()>0.5?1:-1;
    let directionY=Math.random()>0.5?1:-1;

    setInterval(()=>{
        let top=parseFloat(sai.style.top);
        let left=parseFloat(sai.style.left);
        if(top>=85 || top<=0) directionY*=-1;
        if(left>=90 || left<=0) directionX*=-1;
        sai.style.top=top+directionY*0.2+"%";
        sai.style.left=left+directionX*0.2+"%";
    },50);
}
...
window.onload=()=>{
    startRain();
    createFloatingSai();
    typeWriter(document.getElementById("questionText"),"Sai… will you be my Valentine? 💌✨");
};
...
document.getElementById("noBtn").onclick=()=>{
    ...
    if(noClick<=3){
        ...
        sadText.style.fontWeight="bold";
        sadText.style.textShadow="0 0 15px white,0 0 35px white";
    }
    else{
...
function openDay(day){
    ...
    function showPoemLine(){
        if(poemIndex >= poems[day].length) return;
        typeWriter(poem, poems[day][poemIndex], 50);
        poemIndex++;
        setTimeout(showPoemLine, 2500);
    }
...
function finalProposal(){
    ...
    confetti({particleCount:500, spread:200, origin:{y:.6}});
}
...
function startFinalProposal(){
    ...
    ">
        <h1 id="proposalText" style="
            font-size:60px;opacity:0;transform:scale(0.5);transition:1.5s;
            text-shadow:0 0 25px pink,0 0 60px red;
        ">Sai 💖</h1>
        <h2 id="marryText" style="
            font-size:80px;margin-top:20px;opacity:0;transition:2s;
    ...
    playMusic("assets/music/happy.mp3");
...
function acceptProposal(){
    ...
    ">
        ...
        <img src="assets/gifs/happy.gif" style="width:300px;margin-top:20px;">
    </div>`;
    playMusic("assets/music/happy.mp3");
}
